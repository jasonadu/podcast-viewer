...
读取 JSON 数组 (含 mp3 / link 字段), 并发下载 mp3,
文件名优先取 mp3 URL 最后一段 (如 737-owen-mcgirr-software-accessibility.mp3),
取不到时回退 page/link URL 最后一个 / 后的 slug + .mp3
...
_g_aria2c = None
# 正在下载中的文件名集合 (防止并发重复下载同名文件)
_g_inflight = set()
_g_name_lock = threading.Lock()
...
def _filename_from_url(url: str) -> str:
    """从 URL 路径最后一段取文件名 (去查询参数, URL 解码, 去非法字符).
    如 https://.../737-owen-mcgirr-software-accessibility.mp3?dest-id=23379
    -> 737-owen-mcgirr-software-accessibility.mp3
    无扩展名时补 .mp3; 取不到返回空字符串."""
    if not url:
        return ""
...
def download_one(item: dict, out_dir: str, tracker: ProgressTracker, force: bool = False) -> dict:
    mp3_url = (item.get("mp3") or "").strip()
    page = (item.get("page") or item.get("link") or "").strip()
    if not mp3_url:
        return {"status": "fail", "name": "", "url": mp3_url, "error": "mp3 url 为空"}

    # 文件名: 优先 mp3 URL 最后一段 (如 737-owen-mcgirr-software-accessibility.mp3),
    # 回退 page/link slug, 最后 untitled
    stem = _filename_from_url(mp3_url) or _slug_from_page(page) or "untitled"
    ...
    with _g_name_lock:
        # 已存在且非空且无 .aria2 控制文件 -> 跳过 (有控制文件说明未下载完, 允许续传)
        if (not force
                and os.path.exists(filepath)
                and os.path.getsize(filepath) > 0
                and not os.path.exists(filepath + ".aria2")):
            tracker.finish(filename, "skip", "已存在")
            return {"status": "skip", "name": filename, "url": mp3_url, "error": ""}
        # 同一文件名已在其他线程下载中 -> 跳过 (防并发重复下载)
        if filename in _g_inflight:
            tracker.finish(filename, "skip", "并发重复任务")
            return {"status": "skip", "name": filename, "url": mp3_url, "error": ""}
        _g_inflight.add(filename)
    try:
...
def main():
    ...
    if _g_stop.is_set():
        ...
        # 清理 .part 残留; .aria2 控制文件保留, 下次运行可断点续传
        cleaned = 0
